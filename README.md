...
| [test-design](skills/test-design/SKILL.md) | SOP for designing and running software tests at the right depth — test scenarios, test case checklists, test plans, coverage audits, bug reports. 4 levels (L1 smoke → L4 critical) × 12 work-type presets, 5 path types (happy/alternate/error/edge/security), plus scripts that scaffold the doc set and generate/verify checklists from the criteria. **Invoke explicitly (`/test-design`)** — measured trigger rate on advisory phrasings ("how do I write acceptance criteria?") is low, because Claude answers those directly; it fires reliably on explicit "write test cases for X" asks. |
